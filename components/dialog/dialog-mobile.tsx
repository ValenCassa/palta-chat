...
const dialogMobileContentVariants = tv({
  slots: {
    ...
    content:
      "bg-popover fixed bottom-0 left-0 right-0 flex max-h-[80vh] flex-col rounded-t-[10px] outline-none",
  },
});
  ...
  return (
    <Drawer.Portal>
      ...
      <Drawer.Content className={content({ className })} {...rest} ref={ref}>
        <div className="mx-auto mb-8 mt-4 h-1.5 w-12 flex-shrink-0 rounded-full bg-olive-300" />
        {children}
      </Drawer.Content>
    </Drawer.Portal>
  );
